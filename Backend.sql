...
-- Query 1:
SELECT Relationship, COUNT(*) AS Total, MAX(VictimID) AS MaxVictimRef
FROM Petitioner
GROUP BY Relationship;

-- Query 2: Find all criminals with their crime details
SELECT c.Name, c.CNIC, c.PrevCases, cr.Offence, cr.Description, cr.PenalCode
FROM Criminal c
JOIN Crime cr ON c.CrimeID = cr.CrimeID;

-- Query 3: Display criminals who committed cyber crimes or harassment
SELECT c.Name, c.CNIC, c.PrevCases, cr.Offence, cr.Description
FROM Criminal c
JOIN Crime cr ON c.CrimeID = cr.CrimeID
WHERE cr.Offence IN ('CyberCrime', 'Harassment');

--4.Show all crimes with penal code greater than 300
SELECT * FROM Crime
WHERE PenalCode > 300;

-- Query 6: Find repeat offenders (more than 10 previous cases) and their current crimes
SELECT c.Name, c.PrevCases, c.CNIC, cr.Offence, cr.PenalCode
FROM Criminal c
JOIN Crime cr ON c.CrimeID = cr.CrimeID
WHERE c.PrevCases > 10;

--Query 5.List all victims who are above 20 years of age
SELECT Fname, Lname, Age FROM Victim
WHERE Age > 20;

-- Query 9: Find all criminals involved in violent crimes (Murder, Rape, Kidnapping)
SELECT c.Name, c.CNIC, c.PrevCases, cr.Offence, cr.PenalCode
FROM Criminal c
LEFT JOIN Crime cr ON c.CrimeID = cr.CrimeID
WHERE cr.Offence IN ('Murder', 'Rape', 'Kidnapping');
	
SELECT Relationship, COUNT(*) AS TotalPetitioners
FROM Petitioner
GROUP BY Relationship
HAVING COUNT(*) >= 2
ORDER BY TotalPetitioners DESC;
